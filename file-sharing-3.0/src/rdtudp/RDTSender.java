...
/**
 * RDTSender receives a byte array from "above"(FileSender), construct a data packet, and
 * send it via UDPSender. It also receives ack packets from UDPSender.
 */
public class RDTSender {
	...
	RDTSender(String hostname, int port) throws IOException {
		try {
			udt = new UDPSender(hostname, port);
	...
	/**
	 * send() delivers the given array of bytes reliably and should not return
	 * until it is sure that the packet has been delivered.
	 */
	void send(byte[] data, int length) throws IOException, ClassNotFoundException {
		...
		while (ack.isCorrupted == true || ack.ack != seqNumber) {
			// Which means do nothing in this case
			ack = udt.recv();
		}
	...
	/**
	 * Swap the bit around i.e. 1 -> 0, 0 -> 1
	 */
	...
	/**
	 * close() is called when there is no more data to send. This method creates
	 * an empty packet with 0 bytes and send it to the receiver, to indicate
	 * that there is no more data.
	 * 
	 * This method should not return until it is sure that the empty packet has
	 * been delivered correctly. It catches any EOFException (which signals the
	 * receiver has closed the connection) and close its own connection.
	 */
	void close() throws IOException, ClassNotFoundException {
		...
		// start timer
		Timer timeOut = new Timer();
		timeOut.schedule(new senderWithTimer(p), 500, 500);
	...
	class senderWithTimer extends TimerTask {
		...
		/**
		 * run() is called when timer timeouts. Packet is retransmitted and
		 * timer is restarted.
		 */
		public void run() {
			try {
