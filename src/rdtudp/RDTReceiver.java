...
/**
 * RDTReceiver receives a data packet from "below"(UDPReceiver) and pass the byte array
 * contained it it to the "above"(FileReceiver).
 */
public class RDTReceiver {
	...
	/**
	 * recv() reads the next in-order, uncorrupted data packet from the layer
	 * below and returns the byte array contains the data. It returns null if
	 * when there is no more data to read (i.e., the transmission is complete).
	 */
	byte[] recv() throws IOException, ClassNotFoundException {
		DataPacket p = udt.recv();

		// validate received pkt and send respective ack
		while (p.isCorrupted == true || p.seq != seqNumber) { 
			// either pkt is corrupted or has wrong seqNumber
			// send previous ack
