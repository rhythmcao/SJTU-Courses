...
public class Env {
	...
	public void initVEnv() 
	{
		...
		sym = Symbol.symbol("stringEqual");
		formals = new RECORD(Symbol.symbol("str1"), new STRING(), new RECORD(Symbol.symbol("str2"), new STRING(), null));
		result = new INT();
		level = new Level(root, sym, new BoolList(true, new BoolList(true, null)));
		vEnv.put(sym, new StdFuncEntry(level, new Temp.Label(sym), formals, result));
		stdFuncSet.add(sym);
		
		sym = Symbol.symbol("stringCompare");
		formals = new RECORD(Symbol.symbol("str1"), new STRING(), new RECORD(Symbol.symbol("str2"), new STRING(), null));
		result = new INT();
		level = new Level(root, sym, new BoolList(true, new BoolList(true, null)));
		vEnv.put(sym, new StdFuncEntry(level, new Temp.Label(sym), formals, result));
